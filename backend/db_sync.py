...
Routes to psycopg2 (Supabase/Postgres) when DATABASE_URL is set,
otherwise falls back to sqlite3 for local dev.
"""
import os
import re
...
def _sqlite_conn():
    import sqlite3
    return sqlite3.connect("meal_tracker.db")
    ...
    try:
        if _USE_PG:
            ...
            if brand:
                cur.execute(
                    "SELECT calories_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g "
                    "FROM ingredient_cache WHERE name=? AND brand=? LIMIT 1",
                    (name, brand),
                )
            else:
                cur.execute(
                    "SELECT calories_per_100g, protein_per_100g, carbs_per_100g, fat_per_100g "
                    "FROM ingredient_cache WHERE name=? AND brand IS NULL LIMIT 1",
                    (name,),
                )
            row = cur.fetchone()
            conn.close()
